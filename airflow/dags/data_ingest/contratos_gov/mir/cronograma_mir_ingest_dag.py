...
@dag(
    dag_id="cronograma_mir_ingest_dag",
    schedule="@daily",
    start_date=datetime(2023, 1, 1),
    catchup=False,
    default_args=default_args,
    description="Ingere o cronograma de cada contrato do MIR já ingerido em contratos_gov.raw_contratos.",
    tags=["sistema:contratos_gov", "dominio:contratacoes", "orgao:mir"],
)
def cronograma_mir_dag() -> None:
    @task
    def fetch_and_store() -> dict:
        try:
            ...
            raise RuntimeError(
                ...
                "Execute contratos_mir_ingest_dag antes de cronograma_mir_ingest_dag."
            ) from exc
        ...
        for contrato_id in contrato_ids:
            try:
                itens = api.get_cronograma_by_contrato_id(str(contrato_id)) or []
            except Exception as exc:
                logging.error(
                    "[cronograma_mir_ingest_dag] Erro no contrato %s: %s",
                    contrato_id,
                    exc,
                )
                continue
        ...
        logging.info("[cronograma_mir_ingest_dag] total=%s", total)
