...
@dag(
    dag_id="programas_especiais_ingest_dag",
    schedule="@daily",
    start_date=datetime(2024, 1, 1),
    catchup=False,
    default_args=default_args,
    description=(
        "Ingere os programas de transferências especiais (emendas parlamentares "
        "individuais) da API do TransfereGov para "
        "transferegov_emendas.raw_programas_especiais. Fonte 100% nacional, sem "
        "filtro de órgão."
    ),
    tags=["sistema:transferegov_emendas", "dominio:emendas_parlamentares"],
)
def programas_especiais_ingest_dag() -> None:
    @task
    def fetch_and_store() -> dict:
        api = ClienteTransfereGov()
        registros = api.get_all_programas_especiais(page_size=1000)

        if not registros:
            logging.warning(
                "[programas_especiais_ingest_dag] Nenhum programa encontrado."
            )
            return {ENTIDADE: 0}

        write_raw(SISTEMA, ENTIDADE, registros, primary_key=PRIMARY_KEY)
        logging.info("[programas_especiais_ingest_dag] %s registro(s).", len(registros))
