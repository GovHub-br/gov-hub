...
ENTIDADE = "programacao_financeira"
...
@dag(
    dag_id="programacao_financeira_mir_ingest_dag",
    schedule="@daily",
    start_date=datetime(2023, 1, 1),
    catchup=False,
    default_args=default_args,
    description=(
        "Ingere a programação financeira de cada plano de ação de TED do MIR "
        "já ingerido em transferegov_ted.raw_planos_acao."
    ),
    ...
    def fetch_and_store() -> dict:
        try:
            ...
            raise RuntimeError(
                ...
                "programacao_financeira_mir_ingest_dag."
            ) from exc
        ...
        for id_plano_acao in plano_acao_ids:
            try:
                itens = (
                    api.get_programacao_financeira_by_id_plano_acao(str(id_plano_acao))
                    or []
                )
            except Exception as exc:
                logging.error(
                    "[programacao_financeira_mir_ingest_dag] Erro no plano de "
                    "ação %s: %s",
                    id_plano_acao,
                    exc,
                )
            ...
            if not itens:
                continue
            write_raw(SISTEMA, ENTIDADE, itens, primary_key=["id_programacao"])
            total += len(itens)

        logging.info("[programacao_financeira_mir_ingest_dag] total=%s", total)
