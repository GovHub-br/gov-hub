-- Gold do MIR: mesma estrutura de resumo_convenios.sql, restrita a Termos de
-- Fomento (base termo_fomento_consolidado).
--
-- Portado de data-application-mir (siconv_dbt/gold/resumo_termos_fomento.sql).
...
with
    base as (select * from {{ ref("termo_fomento_consolidado") }}),
    desembolso as (
        select
            nr_convenio,
...
select
    -- Identificação
    b.nr_convenio,
    -- Mesma leitura de resumo_convenios.sql: o grão aqui é o termo de fomento
    -- (todas as CTEs já vêm agregadas por nr_convenio e entram por left join),
    -- a UG é atributo e não chave de agregação. A dupla contagem vinha da
    -- Silver (convenios_consolidados devolvia uma linha por convênio × UG, e
    -- termo_fomento_consolidado herdava a multiplicação); corrigida lá, aqui
    -- só propagamos as UGs consolidadas e a quantidade.
