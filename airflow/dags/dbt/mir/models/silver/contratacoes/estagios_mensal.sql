...
-- Silver do MIR: estágios mensais de execução financeira do SIAFI
-- (empenhado/liquidado/pago/restos a pagar), parseados a partir dos
-- empenhos do tesouro.
--
-- Cruza dois sistemas (contratos_gov + tesouro_gerencial, via
-- empenhos_tesouro_mir) — vive no projeto do órgão, não em nenhum pacote de
-- sistema (ADR-0004/0009).
--
-- Portado de data-application-mir (contratos_dbt/silver/estagios_mensal.sql).
-- Granularidade: uma linha por (ne, cnpj_cpf, info_complementar,
-- mes_lancamento).
with
    ...
    parsed_estagios as (
        select
            ...
            case
                when emissao_dia like '000/%' then '01' else substring(emissao_dia, '\/(\d{2})\/')
    ...
    processo_fixed as (
        select
            ...
            ano_lancamento,
            case
                when eh_rap then array[ano_lancamento - 1, ano_lancamento] else array[ano_lancamento]
            end as ano_efetivo,
    ...
    fix_data as (
        select
            ...
            info_complementar,
            eh_rap,
            case when ano_efetivo = ano_lancamento then mes_lancamento else '12' end as mes_lancamento,
            ano_lancamento,
            ano_efetivo,
            num_processo,
            valor_empenhado,
            valor_liquidado,
            valor_pago,
            case when ano_efetivo = ano_lancamento then restos_a_pagar else - restos_a_pagar end as restos_a_pagar,
            restos_a_pagar_pago,
            dt_ingest
        from unnest_rap
    ),
    ...
    results as (
        select
            ...
            info_complementar,
            num_processo,
            to_date(ano_efetivo || '-' || mes_lancamento || '-01', 'YYYY-MM-DD') as mes_lancamento,
            valor_empenhado,
            valor_liquidado,
            valor_pago,
            restos_a_pagar,
