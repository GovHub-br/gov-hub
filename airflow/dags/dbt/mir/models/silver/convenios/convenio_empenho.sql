-- Silver do MIR: convênio com seus empenhos (SICONV, não confundir com os
-- empenhos do SIAFI já usados em convenios_consolidados). Cruza o pacote
-- siconv (via convenios_consolidados), por isso vive em dbt/mir/ (ADR-0004).
--
-- Portado de data-application-mir (siconv_dbt/silver/convenio_empenho.sql),
-- sem alteração de lógica de negócio — só o `ref()` da entidade do SICONV
-- mudou de projeto.
with
    convenio as (select * from {{ ref("convenios_consolidados") }}),
    empenho as (select * from {{ ref("siconv", "empenho") }})
