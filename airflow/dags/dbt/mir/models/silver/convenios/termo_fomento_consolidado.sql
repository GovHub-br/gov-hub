-- Silver do MIR: mesma junção de proposta_convenio.sql, restrita à
-- modalidade Termo de Fomento — base do resumo_termos_fomento (Gold). Cruza
-- o pacote siconv (via convenios_consolidados), por isso vive em dbt/mir/
-- (ADR-0004).
--
-- Portado de data-application-mir
-- (siconv_dbt/silver/termo_fomento_consolidado.sql), sem alteração de lógica
...
select
    ...
    c.nr_processo,
    c.ug_emitente,
    -- UG(s) já consolidadas em atributo por convenios_consolidados (uma linha
    -- por nr_convenio). qtd_ugs_responsaveis é propagado para que o Gold
    -- consiga sinalizar os instrumentos executados por mais de uma UG sem
    -- voltar à Silver.
    c.ug_responsavel_codigo,
    c.ug_responsavel_nome,
    c.qtd_ugs_responsaveis,
    c.dia_publ_conv,
...
where p.modalidade = 'TERMO DE FOMENTO'
