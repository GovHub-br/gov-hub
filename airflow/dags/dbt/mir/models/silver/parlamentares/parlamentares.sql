...
-- Silver do MIR: visão unificada de deputados e senadores em exercício, com
-- partido canonicalizado e logo do partido.
--
-- Cruza dois sistemas (camara_deputados + senado_federal) — vive no projeto
-- do órgão, não em nenhum pacote de sistema (ADR-0004/0009), porque nenhum
-- dos dois pacotes pode depender do outro sem criar acoplamento entre
-- sistemas que, por natureza, não têm relação alguma um com o outro.
--
-- Portado de data-application-mir
-- (dados_abertos_dbt/silver/parlamentares.sql), sem alteração de lógica de
-- negócio — só os `ref()` mudaram de projeto/pacote.
with
    ...
    sigla_map as (
        select trim(upper(sigla_origem)) as sigla_origem, trim(upper(sigla_canonica)) as sigla_canonica
    ...
    partidos_logo as (select trim(upper(sigla)) as chave_join_sigla_partido, logo_url from {{ ref("partidos_logo") }})
