...
-- Silver do MIR: notas de crédito unificadas (pré/pós-2026) enriquecidas com
-- programa/ação de governo, resolvidos via PTRES quando a própria nota de
-- crédito não os traz preenchidos (comum nas linhas pós-2026).
--
-- Cruza tesouro_gerencial (nc_tesouro_unificado, pf_ptres) — por isso vive em
-- dbt/mir/, não no pacote tesouro_gerencial (ADR-0004/0009).
--
-- Portado de data-application-mir (dags/dbt/mir/models/empenhos_ted_dbt/
-- silver/nc_unificado.sql). nc_tesouro_mir vira ref("tesouro_gerencial",
-- "nc_tesouro_unificado"); pf_ptres vira ref("tesouro_gerencial", "pf_ptres")
-- — ambos migrados como Silver de tesouro_gerencial (sem cruzamento de
-- sistema em si).
with
    nc_tesouro as (select * from {{ ref("tesouro_gerencial", "nc_tesouro_unificado") }}),

    programa_por_ptres as (
        select ptres, programa_governo, programa_governo_descricao, acao_governo, plano_orcamentario_descricao_6
        from
            (
                select
                    ptres,
                ...
                from {{ ref("tesouro_gerencial", "pf_ptres") }}
            ) subquery
        where rn = 1
    )
