...
-- Silver do MIR: une programação financeira do SIAFI (pf_tesouro) com a
-- programação financeira do TED (pf_transfere), casadas pela chave textual
-- comum (últimos 12 caracteres de `pf`, no SIAFI; `tx_numero_programacao`,
-- no TED).
--
-- Cruza tesouro_gerencial (pf_tesouro) e transferegov_ted (pf_transfere) —
-- por isso vive em dbt/mir/, não em nenhum dos dois pacotes (ADR-0004/0009).
--
-- Portado de data-application-mir (dags/dbt/mir/models/empenhos_ted_dbt/
-- silver/pf_unificado.sql). pf_tesouro vira ref("tesouro_gerencial",
-- "pf_tesouro"); pf_transfere vira ref("transferegov_ted", "pf_transfere").
with
    pf_tesouro as (
        select
            emissao_mes,
        ...
        from {{ ref("tesouro_gerencial", "pf_tesouro") }}
    ),
    ...
    pf_transfere as (
        ...
        from {{ ref("transferegov_ted", "pf_transfere") }}
    )
