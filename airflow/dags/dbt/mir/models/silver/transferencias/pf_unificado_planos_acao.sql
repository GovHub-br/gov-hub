...
-- Silver do MIR: pf_unificado enriquecido com os atributos do plano de ação
-- de TED (deduplicado pela ingestão mais recente por id_plano_acao).
--
-- Cruza transferegov_ted (planos_acao_ted); pf_unificado já vive neste mesmo
-- projeto (dbt/mir/models/silver/transferencias/), então é ref simples.
--
-- Portado de data-application-mir (dags/dbt/mir/models/empenhos_ted_dbt/
-- silver/pf_unificado_planos_acao.sql). planos_acao_ted vira ref(
-- "transferegov_ted", "planos_acao_ted").
with
    pf_unificado as (select * from {{ ref("pf_unificado") }}),
...
select
    ...
    pa.sq_instrumento,
    pa.aa_instrumento,
    coalesce(greatest(pf.dt_ingest, pa.dt_ingest_plano_acao), pf.dt_ingest, pa.dt_ingest_plano_acao) as dt_ingest
