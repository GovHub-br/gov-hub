...
DAGS_FOLDER = (
    Path(__file__).resolve().parents[2] / "airflow" / "dags" / "data_ingest" / "ppa"
)
...
class TestPpaDagsIntegrity:
    ...
    def test_ppa_dag_has_31_ingest_tables(self, dagbag: DagBag) -> None:
        """31 entidades × 3 revisões (2024-2026) = 93 tasks de ingestão."""
        dag = dagbag.dags["ppa_ingest_dag"]
        ingest_tasks = [t for t in dag.task_ids if t.startswith("ingerir_")]
        assert len(ingest_tasks) == 93, len(ingest_tasks)
