...
DAGS_FOLDER = (
    Path(__file__).resolve().parents[2] / "airflow" / "dags" / "data_ingest" / "siconv"
)
...
class TestSiconvDagsIntegrity:
    ...
    def test_siconv_dag_has_16_ingest_tables(self, dagbag: DagBag) -> None:
        """1 zip nacional, 16 entidades — uma task de ingestão por entidade."""
        dag = dagbag.dags["siconv_ingest_dag"]
        ingest_tasks = [t for t in dag.task_ids if t.startswith("ingerir_")]
        assert len(ingest_tasks) == 16, len(ingest_tasks)
