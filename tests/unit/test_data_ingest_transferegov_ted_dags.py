...
class TestTransfereGovTedDagsIntegrity:
    ...
    def test_all_dags_have_orgao_mir_tag(self, dagbag: DagBag) -> None:
        # As 4 DAGs vivem em data_ingest/transferegov_ted/mir/: `programas` tem
        # filtro real de sigla, as demais escopo indireto (iteram sobre os
        # id_programa/id_plano_acao já ingeridos) — ver catalogo/sistemas/
        # transferegov_ted.yml.
        missing = [
            dag_id for dag_id, dag in dagbag.dags.items() if "orgao:mir" not in dag.tags
        ]
        assert not missing, f"DAGs sem tag orgao:mir: {missing}"
